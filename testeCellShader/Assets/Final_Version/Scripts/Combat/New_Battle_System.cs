...
public class New_Battle_System : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (Scene_Variables.instance != null)
        {
            ...
            {
                Unit_Info unitInfo = playerUnitsPosition[u].GetComponent<Select_Unit>().unitInPos.GetComponent<Unit_Info>();

                unitInfo.unitLevel = Scene_Variables.instance.playerLevel;
                unitInfo.lifeCur = unitInfo.lifeBase + unitInfo.lifeScaling * (unitInfo.unitLevel - 1) + unitInfo.lifeBonus;
            }

            for (int u = 0; u < enemyUnitsPosition.Length; u++)
            {
                Unit_Info unitInfo = enemyUnitsPosition[u].GetComponent<Select_Unit>().unitInPos.GetComponent<Unit_Info>();

                unitInfo.unitLevel = Scene_Variables.instance.enemyLevel;
                unitInfo.lifeCur = unitInfo.lifeBase + unitInfo.lifeScaling * (unitInfo.unitLevel - 1) + unitInfo.lifeBonus;
            }
        }
    }
